// --------------------------------------------------------------
//        pour gestion de l'affichage ou non des sous cat
// --------------------------------------------------------------

function gestionAfficher(id_sous_cat, afficher_sous_categorie) {
    // création de l'objet XHLHttpRequest
    const xmlhttp = new XMLHttpRequest(); 
    
    xmlhttp.open("POST", "controler/requete_gestion_affichage_sscat.php", true); // il va chercher le fichier et l'ouvre
    ...
    xmlhttp.onload = function() { // peut aussi utiliser onreadystatechange ou autre chose
        ...
        let resultat = document.getElementById(id_sous_cat); // récupère le td contenant l'input
        
        for (let data in myobj) {
            
            resultat.value = myobj[data].afficher_sous_categorie;

            if (myobj[data].afficher_sous_categorie == 1) {
    ...
    data = ('afficher_sous_categorie='+ afficher_sous_categorie +'&id_sous_cat='+ id_sous_cat);
